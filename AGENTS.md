...
- Use `deploy/runtime-artifact.json` and `docs/runtime-artifact-contract.md`.
- Build exact source under Linux ARM64 with the pinned Node/npm, then independently verify paths, hashes, production dependencies and native bindings.
- Require the isolated unpacked-runtime receipt before publishing assets. A source build or development-checkout smoke does not pass packaging acceptance.
- Preserve existing service paths, runtime prefix, external MongoDB/session data, Quotes service separation, environment files and rollback targets. Never bundle writable state or secrets.
