...
const adminDirectory = useDirectory<Admin>("/admins", () => Boolean(currentAdmin.value));
const tutorDirectory = useDirectory<Tutor>("/tutors/all", () => Boolean(currentAdmin.value));
const userDirectory = useDirectory<User>("/users/all", () => Boolean(currentAdmin.value));
const { items: admins } = adminDirectory;
const { items: tutors } = tutorDirectory;
const { items: users } = userDirectory;
const error = ref("");
const message = ref("");
...
watch(
	() => currentAdmin.value?.editAdmins,
	(allowed) => {
		if (!allowed) Object.assign(newAdmin, { name: "", email: "", password: "", editAdmins: false });
	},
	{ immediate: true, flush: "sync" }
);
useUnsavedChanges(
	() => actionPending.value || Boolean(newAdmin.name || newAdmin.email || newAdmin.password || newAdmin.editAdmins)
);
	...
	<section class="Signup text-center">
		...
		<div v-if="currentAdmin" class="tutorList mt-2">
			...
			<ul>
				...
				<ProfileFields
					:editing="adminEdit"
					:entity="adminEdit ? profileDraft : currentAdmin"
					:fields="adminFields"
				/>
			...
			<button
				...
				@click="adminEdit ? saveAdmin(profileDraft) : toggleAdmin()"
			>
				{{ adminEdit ? "Save" : "Edit" }}
			</button>
		</div>
		...
		<h2>Admins</h2>
		<DirectoryControls
			v-model="adminDirectory.query.value"
			label="admins"
			:loading="adminDirectory.loading.value"
			:error="adminDirectory.error.value"
			:next="Boolean(adminDirectory.nextCursor.value)"
			:previous="Boolean(adminDirectory.previousCursor.value)"
			@search="adminDirectory.search"
			@next="adminDirectory.next"
			@previous="adminDirectory.previous"
			@refresh="adminDirectory.refresh"
		/>
		<p v-if="!adminDirectory.loading.value && !adminDirectory.error.value && !adminDirectory.items.value.length">
			No matching admins.
		</p>
		<form v-if="currentAdmin?.editAdmins" class="admin-create" @submit.prevent="createAdmin">
			...
			<label>
				Name
				<input v-model="newAdmin.name" :disabled="actionPending" maxlength="100" required />
			</label>
			<label>
				Email
				<input
			...
			<label>
				<input v-model="newAdmin.editAdmins" :disabled="actionPending" type="checkbox" />
				Grant admin-management privilege
			</label>
			<button class="btn-primary btn" type="submit" :disabled="actionPending">Create admin</button>
		</form>
		...
		<h2>Tutors</h2>
		<DirectoryControls
			v-model="tutorDirectory.query.value"
			label="tutors"
			:loading="tutorDirectory.loading.value"
			:error="tutorDirectory.error.value"
			:next="Boolean(tutorDirectory.nextCursor.value)"
			:previous="Boolean(tutorDirectory.previousCursor.value)"
			@search="tutorDirectory.search"
			@next="tutorDirectory.next"
			@previous="tutorDirectory.previous"
			@refresh="tutorDirectory.refresh"
		/>
		<p v-if="!tutorDirectory.loading.value && !tutorDirectory.error.value && !tutorDirectory.items.value.length">
			No matching tutors.
		</p>
		...
		<h2>Users</h2>
		<DirectoryControls
			v-model="userDirectory.query.value"
			label="users"
			:loading="userDirectory.loading.value"
			:error="userDirectory.error.value"
			:next="Boolean(userDirectory.nextCursor.value)"
			:previous="Boolean(userDirectory.previousCursor.value)"
			@search="userDirectory.search"
			@next="userDirectory.next"
			@previous="userDirectory.previous"
			@refresh="userDirectory.refresh"
		/>
		<p v-if="!userDirectory.loading.value && !userDirectory.error.value && !userDirectory.items.value.length">
			No matching users.
		</p>
