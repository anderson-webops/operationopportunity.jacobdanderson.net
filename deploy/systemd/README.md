...
8. Build the static artifact with the same full commit and deployment timestamp used in `api.env`: set `SOURCE_REVISION` and `OPPORTUNITY_DEPLOYED_AT` for `npm run -w front-end build`, or pass them as `SOURCE_REVISION` and `OPPORTUNITY_DEPLOYED_AT` Docker build arguments. Do not promote an artifact whose `release.json` contains a null commit or timestamp.
