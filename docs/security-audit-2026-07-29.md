...
The first IPv4 observation served the pre-remediation SSG build: `/release.json` fell through to old SPA HTML and the response retained the prior inline/eval-capable CSP. During follow-up, `/release.json` intermittently returned release `v2.2.0` with null commit and timestamp under the same outer response policy, while the root and API remained unreliable. This exposed a direct-source-build path that did not enforce deployment identity; the release writer and build-security gate now derive or require a valid commit and UTC timestamp and fail closed otherwise. The IPv4 API health endpoint and both tested IPv6 endpoints timed out. Host access was unavailable, so no production promotion is claimed. Source release, CI runner execution, host promotion, database backup, one-time manager recovery, APILayer revocation, and public verification remain distinct gates.
