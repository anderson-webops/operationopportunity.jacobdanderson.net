...
This checklist complements the automated `npm run a11y` axe smoke suite. Run it before shipping home, profile, signup, support, and informational changes.

## Screen Reader Pass

- macOS VoiceOver: open `/`, `/about`, `/profile`, `/signup`, `/supportus`; use `VO + Right Arrow` through each page, then jump by headings, landmarks, buttons, form controls, and links.
...
- Verify visible focus on header navigation, signup controls, profile controls, support links, form fields, buttons, and footer links.
