...
assert(![...frontFiles, ...backFiles].some(file => extname(file) === ".map"), "Production builds must not include source maps");
...
for (const htmlFile of frontFiles.filter(file => extname(file) === ".html")) {
	...
	assert(!/\b(?:SESSION_SECRET|VAULT_SECRET_ID|MONGODB_URI)\b/.test(html), `${htmlFile} contains a server-only setting`);
}
